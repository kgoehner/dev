...
vim.pack.add({ 'https://github.com/mfussenegger/nvim-dap-python' }, { load = true })
vim.pack.add({ 'https://github.com/leoluz/nvim-dap-go' }, { load = true })
vim.pack.add({ 'https://github.com/rcarriga/nvim-dap-ui' }, { load = true })
...
vim.keymap.set('n', '<leader>drm', dapy.test_method, { desc = '[D]ebug [R]un [M]ethod' })
vim.keymap.set('n', '<leader>drf', dapy.test_class, { desc = '[D]ebug [R]un [F]ile' })
...
vim.keymap.set('n', '<leader>du', ':lua local widgets=require("dap.ui.widgets");widgets.centered_float(widgets.scopes)<CR>')
...
vim.keymap.set('n', '<leader>db', ':lua require("dap").set_breakpoint(nil, nil, vim.fn.input("Log point message: "))<CR>')
